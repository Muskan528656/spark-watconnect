...
module.exports = (app) => {
  ...
  router.post(
    ...
    async (req, res) => {
      try {
        ...
        const formattedCompany = `${req.body.company
          .toLowerCase()
          .replace(/[^a-zA-Z0-9 ]/g, "")
          .replace(/\s+/g, "_")}`;
        ...
        if (formattedCompany) {
          ...
          if (result.includes(formattedCompany)) {
            return res
              .status(200)
              .json({
                errors: "A company is already registered with this name.",
              });
          }
        }
        if (req.body.status === "Closed - Converted") {
          ...
          const reqBody = {
            ...
            user_info: {
              ...
              country_code: req.body.country_code
                ? String(req.body.country_code).trim()
                : "+91",
            },
            invoice: {
          ...
          const result = await Company.createCompanyWithUser(
            reqBody,
            cryptPassword
          );
          if (!result) {
            return res
              .status(400)
              .json({
                errors: "Something went wrong while converting company",
              });
          } else {
            req.body.convertedcompanyid = result.company_id;
            // await Company.sendeMail(req.body.email, password);
            if (req.body?.invoice?.planname.toLowerCase() === "free") {
              await Company.sendeMail(req.body.email, password);
            }
          }
        }
  ...
  router.put(
    ...
    async (req, res) => {
      ...
      try {
        if (req.body.email) {
          const isDuplicate = await Lead.checkEmailExists(
            req.body.email,
            leadId
          );
          if (isDuplicate) {
        ...
        if (updatedLead) {
          ...
          return res
            .status(400)
            .json({ success: false, errors: "Failed to update lead" });
        }
      } catch (error) {
