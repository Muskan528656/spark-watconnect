...
module.exports = (app) => {
  ...
  router.post("/", async (req, res) => {
    try {
      ...
      for (const f in req.files) {
        ...
        const uploadPath =
          process.env.FILE_UPLOAD_PATHH +
          "/" +
          requestBody["company_info"]["tenantcode"];
        filePath = `${uploadPath}/${fileDetail.name}`;
        filePathObj[f] = filePath;

        // Create upload directory if not exists
        if (!fs.existsSync(uploadPath) && times === 0) {
          fs.mkdirSync(uploadPath, { recursive: true });
          times++;
        }

        // Move file using Promise
      ...
      const password = requestBody["user_info"]["password"];
      ...
      const reqBody = {
        ...requestBody,
        company_info: {
          ...requestBody["company_info"],
          logourl: fPath,
        },
      };
      console.log("reqBodyreqBody=>>>>>>>>>>>>>>>", reqBody);
      const result = await Company.createCompanyWithUser(reqBody, cryptPassword);
      console.log("createCompanyWithUser", result);
      if (!result) {
        return res.status(400).json({ errors: "Bad Request" });
      }

      if (
        requestBody?.invoice?.planname &&
        requestBody.invoice.planname.toLowerCase() === "free"
      ) {
        const emailData = {
          name: requestBody["company_info"]["tenantcode"],
          username: requestBody["user_info"]["email"],
          password: requestBody["user_info"]["password"],
          url: process.env.BASE_URL,
        };
        console.log("emailDataemailData", emailData);
        const regMailStatus = await sendEmail(
          requestBody["user_info"]["email"],
          emailData,
          null,
          "register_mail"
        );
  ...
  router.put("/updateCompanyWithUser/", fetchUser, async (req, res) => {
    ...
    await Company.updateCompanyWithUser(reqBody)
      .then((result) => {
        if (!result) {
          return res.status(400).json({ errors: "Bad Request" });
        }
  ...
  router.get("/send/mail/", fetchUser, async (req, res) => {
    let result = await Company.sendeMail(
      "abhishek.sharma@gmail.com",
      "hkh@234"
    );
  ...
  router.post("/createcompany", async (req, res) => {
    ...
    requestBody["company_info"]["logourl"] = fPath;
    await Company.createCompanyWithUser(reqBody, cryptPassword)
      .then((result) => {
        if (!result) {
          return res.status(400).json({ errors: "Bad Request" });
        }
