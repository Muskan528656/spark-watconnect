...
const Invoice = require("../../models/watconnect/invoice.model.js");
const permissions = require("../../constants/permissions.js");
const bcrypt = require("bcryptjs");

const crypto = require("crypto");
const Mailer = require("../../models/watconnect/mail.model.js");
const moment = require("moment");
const Auth = require("../../models/watconnect/auth.model.js");
function generateHmacSha256(message, secret) {
  return crypto.createHmac("sha256", secret).update(message).digest("hex"); // You can also use 'base64' if you prefer
}
...
module.exports = (app) => {
  ...
  router.get("/company/:id", fetchUser, async (req, res) => {
    ...
    if (result) {
      const planResult = await Invoice.getPlanPriceByValidity(
        result[0].plan_id,
        result[0].validity
      );
      if (planResult && planResult.length > 0) {
        result[0].plan_price = planResult[0].price;
      }
      res.status(200).json(result);
    } else {
      res.status(400).json({ errors: "No data" });
  ...
  router.put("/updateInvoiceAddTrans/:id", fetchUser, async (req, res) => {
    ...
    const isFirstInvoice = !(await Invoice.hasExistingInvoices(
    ...
    if (result) {
      ...
      let body = `<html><body style="font-family: system-ui;"><div style="padding: 0px 10px;background-color: #F9F9F9;border: 2px solid #E0E0E0;width: 75%; display: block;margin: 0 auto;"><p>Dear${" " + companyData.firstname + " " + companyData.lastname
        },</p><p>Thank you for your payment!</p><p>We are pleased to inform you that your payment of ${invoiceData.amount
        } for <b>Watconnect</b> has been successfully processed on ${moment(
          invoiceData.date
        ).format(
          "DD-MM-YYYY"
        )}.</p><p>Please find your invoice attached to this email for your records.</p><h4>Payment Details:</h4><ul type="disc"><li>Payment Amount: ${invoiceData.amount
        }</li><li>Payment Date: ${moment(invoiceData.date).format(
          "DD-MM-YYYY"
        )}</li><li>Payment Method: ${invoiceData.payment_method}</li>${invoiceData.payment_method !== "cash"
          ? `<li>Transaction ID: ${invoiceData.transaction_cheque_no || ""
          }</li>`
          : ""
        }
      </ul>${invoiceData.payment_method !== "cash"
          ? `<h4>Order Details:</h4><ul type="disc"><li>Order ID: ${invoiceData.order_id || ""
          }</li><li>Product/Service: <b>Watconnect</b></li></ul>`
          : ""
        }<h4>Order Details:</h4><ul type="disc"><li>Order ID: ${invoiceData.order_id
        }</li><li>Product/Service: <b>Watconnect</b></li></ul><p>If you have any questions or need further assistance, please feel free to contact us at contact@ibirdsservices.com or call us at 9876543210.</p><p>Thank you for choosing iBirds Software Services Pvt. Ltd.. We appreciate your business!</p><p>Best regards,<br>iBirds Software Services Pvt. Ltd.<p></div></body></html>`;
      if (isFirstInvoice) {
        try {
          const password =
            companyData.company_name.toLowerCase().replace(/\s+/g, "") +
            "#$@" +
          ...
          console.log("companyData.user_id", companyData.user_id);
          const resultpassword = await Auth.updateById(
            companyData.user_id,
            userRec
          );
          ...
          console.log("companyData.adminemail=>>", companyData.adminemail);
          // await Mailer.sendEmail(companyData.adminemail, emailData, null, 'register_mail');
          const regMailStatus = await Mailer.sendEmail(
          ...
          console.log("[✓] Registration email status:@@@@", regMailStatus);
        } catch (error) {
      ...
      try {
        const invoiceMailStatus = await Mailer.sendEmail(
        ...
        console.log("[✓] Invoice email status:", invoiceMailStatus);
      } catch (error) {
  ...
  router.put("/generateInvoice/AddTrans/:id", fetchUser, async (req, res) => {
    const invoiceId = req.params.id;
    const invoiceData = req.body;
    let companyData = invoiceData.company;
    const isFirstInvoice = !(await Invoice.hasExistingInvoices(
    ...
    // return null;
    const result = await Invoice.generateInvoiceAddTrans(
      invoiceId,
      invoiceData
    );
    if (result) {
      ...
      let subject = "Payment Confirmation";
      let body = `<html><body style="font-family: system-ui;"><div style="padding: 0px 10px;background-color: #F9F9F9;border: 2px solid #E0E0E0;width: 75%; display: block;margin: 0 auto;"><p>Dear${" " + companyData.firstname + " " + companyData.lastname
        },</p><p>Thank you for your payment!</p><p>We are pleased to inform you that your payment of ${invoiceData.billing_amount
        } for <b>Watconnect</b> has been successfully processed on ${moment(
          invoiceData.date
        ).format(
          "DD-MM-YYYY"
        )}.</p><p>Please find your invoice attached to this email for your records.</p><h4>Payment Details:</h4><ul type="disc"><li>Payment Amount: ${invoiceData.billing_amount
        }</li><li>Payment Date: ${moment(invoiceData.date).format(
          "DD-MM-YYYY"
        )}</li><li>Payment Method: ${invoiceData.payment_method}</li>${invoiceData.payment_method !== "cash"
          ? `<li>Transaction ID: ${invoiceData.transaction_cheque_no || ""
      ...
      if (isFirstInvoice) {
        try {
          ...
          // const regMailStatus = await Mailer.sendEmail(
          //   companyData.adminemail,
          //   subject,        // not object — should be a string
          //   body,           // HTML content
          //   "register_mail" // fromEmail
          // );
          // console.log("[✓] Registration email status:", regMailStatus);
        } catch (error) {
          console.error("Error processing first invoice registration:", error);
      ...
      const data = {
        name: companyData.firstname + " " + companyData.lastname,
        amount: invoiceData.billing_amount,
        date: moment(invoiceData.date).format("DD-MM-YYYY"),
        payment_method: invoiceData.payment_method || "--",
        transaction_cheque_no: invoiceData.transaction_cheque_no || "--",
        order_id: invoiceData.order_id || "--",
