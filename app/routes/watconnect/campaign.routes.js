...
module.exports = app => {
    ...
    router.get("/campaign", fetchUser, checkModuleAccess("campaign"), async (req, res) => {
        ...
        console.log("req.userinfo, whatsapp_setting_numberreq.userinfo, whatsapp_setting_number", req.userinfo, whatsapp_setting_number);
        const result = await campaignModel.getRecords(req.userinfo, whatsapp_setting_number);
        console.log("resultcampaign", result);
    ...
    // router.get("/campaign/performing", fetchUser, checkModuleAccess("campaign"), async (req, res) => {
    ...
    //     campaignModel.init(req.userinfo.tenantcode);
    //     // const statusName = req.query.status;
    //     // const result = await campaignModel.getTopPerformingCampaign(whatsapp_setting_number);
    ...
    //  router.get("/campaign/:id", fetchUser, checkModuleAccess("campaign"), async (req, res) => {
    //     campaignModel.init(req.userinfo.tenantcode);
    ...
    router.post("/campaign", fetchUser, checkModuleAccess("campaign"), async (req, res) => {

        const { name, type, status, template_name, start_date, group_ids, lead_ids, business_number } = req.body;
        const errors = [];
        ...
        if (req.body.hasOwnProperty("lead_ids")) {
            campaignRecord.lead_ids = Array.isArray(lead_ids) ? lead_ids : [];
        }
        ...
        console.log("working create ->", result);
    ...
    router.put("/campaign/:id", fetchUser, checkModuleAccess("campaign"), async (req, res) => {
        try {
    ...
    router.get("/campaign/download/:filename", fetchUser, checkModuleAccess("campaign"), async (req, res) => {
        try {
            const filename = req.params.filename;
            let filePath = `${process.env.FILE_UPLOAD_PATHH}/${req.userinfo.tenantcode}/campaign_files/${filename}`;
    ...
    router.post("/campaign/file/:id", fetchUser, checkModuleAccess("campaign"), async (req, res) => {
        ...
        try {
            ...
            for (const fileDetail of Object.values(req.files)) {
                ...
                if (fileInsert) {
                    ...
                    let uploadPath = process.env.FILE_UPLOAD_PATHH + '/' + req.userinfo.tenantcode + '/' + 'campaign_files';
                    // const uploadPath = path.join(__dirname, '..', 'files', 'ibs_ibirds', 'campaign_files');
    ...
    router.post("/campaign/params", fetchUser, checkModuleAccess("campaign"), async (req, res) => {
        campaignModel.init(req.userinfo.tenantcode);
    ...
    router.get("/campaign/params/:id", fetchUser, checkModuleAccess("campaign"), async (req, res) => {
        campaignModel.init(req.userinfo.tenantcode);
    ...
    router.get(
        ...
        async (req, res) => {
            try {
                ...
                campaignModel.init(userinfo.tenantcode);
                const result = await campaignModel.findById(id);

                if (!result) {
                    return res.status(200).json({ success: false, error: "Campaign not found" });
                }
    ...
    router.delete("/campaign/:id", fetchUser, checkModuleAccess("campaign"), async (req, res) => {
        campaignModel.init(req.userinfo.tenantcode);
        const { id } = req.params;
        try {
            const result = await campaignModel.deleteCampaign(id);
