...
module.exports = app => {
    ...
    router.post("/clone", fetchUser, checkModuleAccess("campaign"), async (req, res) => {
        ...
        try {
            const result = await fileModel.insertFileRecords(newFile, req.userinfo.id);
            if (result) {
                return res.status(200).json({ success: true, record: result });
            } else {
                return res.status(400).json({ success: false, errors: "File creation failed" });
            }
        } catch (error) {
    ...
    router.post('/:id', fetchUser, async (req, res) => {
        ...
        try {
            ...
            const fileArray = req.files ? Object.values(req.files) : [];
            const responseArray = [];
            const uploadPath = path.join(process.env.FILE_UPLOAD_PATH_IMAGE, req.userinfo.tenantcode, 'attachment');
