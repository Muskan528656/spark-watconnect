  ...
  router.post("/sendemail", fetchUser, [
    ...
    async (req, res) => {
      ...
      if (!taskRec) {
        ...
        // if (req.body.to) {
        //   const userRec = await Auth.findById(req.userinfo.id); // लॉगइन यूज़र की जानकारी निकाल रहे हैं
        //   let fromAdd = `${userRec.firstname} ${userRec.lastname}<${userRec.email}>`; // 'From' के लिए फॉर्मेट तैयार कर रहे हैं
        //   const res = Mailer.sendEmail(req.body.to, req.body.subject, req.body.editorHtml, fromAdd); // ईमेल भेजने के लिए Mailer function कॉल कर रहे हैं
        //   console.log("REs-", res);
