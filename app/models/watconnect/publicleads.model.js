...
async function createLead(newLead) {
    ...
    INSERT INTO public.leads (
      first_name, last_name, email, mobile_no, country_code, status, company, description, 
...
async function generatePublicLead(body) {
    try {
        ...
        if (result.rows.length > 0) {
            let value = [body.firstname, body.lastname, body.mobile,body.country_code, result.rows[0].id];
            let query = `UPDATE public.leads SET first_name=$1, last_name=$2, mobile_no=$3, country_code=$4 WHERE id = $5 RETURNING *`;
            ...
            let value = [body.firstname, body.lastname, body.email, body.mobile, body.country_code, body.status];
            let query = `INSERT INTO public.leads (first_name, last_name, email, mobile_no, country_code, status)  VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`;
